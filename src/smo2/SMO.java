...
public class SMO extends Classifier implements WeightedInstancesHandler {
	...
	protected class BinarymySMO implements Serializable {
		...
		/**
		 * Fits logistic regression model to SVM outputs analogue to John
		 * Platt's method.
		 *
		 * @param insts
		...
		protected void buildClassifier(Instances insts, int cl1, int cl2,
				boolean fitLogistic, int numFolds, int randomSeed)
				throws Exception {
			...
			while ((numChanged > 0) || examineAll) {
				...
				if (examineAll) {
					 ...
					 * ((m_bUp < m_bLow - 2 * m_tol) && (innerLoopSuccess ==
					 * true)) { innerLoopSuccess = takeStep(m_iUp, m_iLow,
					 * m_errors[m_iLow]); }
					 */
				}
		...
		protected double mySVMOutput(int index, Instance inst) throws Exception {
			...
			if (!m_useRBF && m_exponent == 1.0) {
				...
				for (int i = m_supportVectors.getNext(-1); i != -1; i = m_supportVectors
						.getNext(i)) {
					result += m_class[i] * m_alpha[i]
							* m_kernel.eval(index, i, inst);
				}
			}
			result -= m_b;
		...
		public String toString() {
			...
			try {
				...
				if (!m_useRBF && m_exponent == 1.0) {
					...
					for (int i = 0; i < m_sparseWeights.length; i++) {
						if (m_sparseIndices[i] != (int) m_classIndex) {
							...
							if (!m_checksTurnedOff) {
								text.append(m_data
										.attribute(m_sparseIndices[i]).name()
										+ "\n");
							} else {
				...
				if (numCacheHits >= 0 && numEval > 0) {
					double hitRatio = 1 - numEval * 1.0
							/ (numCacheHits + numEval);
					text.append(" ("
							+ Utils.doubleToString(hitRatio * 100, 7, 3).trim()
							+ "% cached)");
				}
		...
		protected boolean takeStep(int i1, int i2, double F2) throws Exception {
			...
			double alph1, alph2, y1, y2, F1, s, L, H, k11, k12, k22, eta, a1, a2, f1, f2, v1, v2, Lobj, Hobj, b1, b2, bOld;
			...
			for (int i = 0; i < m_alpha.length; i++) {
				...
				if (Utils.eq(m_alpha[i], 0)) {
					if (Utils.sm(m_class[i] * output, 1)) {
						System.err.println("KKT condition 1 violated: "
								+ m_class[i] * output);
					}
				}
				if (Utils.gr(m_alpha[i], 0)
						&& Utils.sm(m_alpha[i], m_C
								* m_data.instance(i).weight())) {
					if (!Utils.eq(m_class[i] * output, 1)) {
						System.err.println("KKT condition 2 violated: "
								+ m_class[i] * output);
					}
				}
				if (Utils.eq(m_alpha[i], m_C * m_data.instance(i).weight())) {
					if (Utils.gr(m_class[i] * output, 1)) {
						System.err.println("KKT condition 3 violated: "
								+ m_class[i] * output);
					}
				}
			}
	 ...
	 * is purely numeric, doesn't contain any missing values, and has a nominal
	 * class. Turning them off also means that no header information will be
	 * stored if the machine is linear. Finally, it also assumes that no
		...
		if (!m_checksTurnedOff) {
			...
			if (insts.classAttribute().isNumeric()) {
				throw new UnsupportedClassTypeException(
						"mySMO can't handle a numeric class! Use"
			...
			for (int i = 0; i < inst.numClasses(); i++) {
				for (int j = i + 1; j < inst.numClasses(); j++) {
					if ((m_classifiers[i][j].m_alpha != null)
						...
						double output = m_classifiers[i][j].mySVMOutput(-1,
								inst);
						if (output > 0) {
							result[j] += 1;
						} else {
				...
				for (int j = i + 1; j < inst.numClasses(); j++) {
					if ((m_classifiers[i][j].m_alpha != null)
						...
						r[i][j] = m_classifiers[i][j].m_logistic
								.distributionForInstance(new Instance(1,
										newInst))[0];
		...
		for (int i = 0; i < numValues; i++) {
			for (int j = i + 1; j < numValues; j++) {
				...
				for (int k = 0; k < numAttributes; k++) {
					attrNames[k] = m_classifiers[i][j].m_data.attribute(k)
							.name();
				}
	...
	public Enumeration listOptions() {
		...
		newVector.addElement(new Option(
				"\tThe complexity constant C. (default 1)", "C", 1,
				"-C <double>"));
		newVector.addElement(new Option("\tThe exponent for the "
				+ "polynomial kernel. (default 1)", "E", 1, "-E <double>"));
		newVector.addElement(new Option(
				"\tGamma for the RBF kernel. (default 0.01)", "G", 1,
				"-G <double>"));
		newVector.addElement(new Option(
		...
		newVector.addElement(new Option(
				"\tUse RBF kernel. " + "(default poly)", "R", 0, "-R"));
		newVector
				.addElement(new Option("\tThe size of the kernel cache. "
						+ "(default 250007, use 0 for full cache)", "A", 1,
						"-A <int>"));
		...
		newVector.addElement(new Option(
				"\tFit logistic models to SVM outputs. ", "M", 0, "-M"));
		newVector.addElement(new Option(
				"\tThe number of folds for the internal\n"
						+ "\tcross-validation. "
						+ "(default -1, use training data)", "V", 1,
				"-V <double>"));
	 ...
	 * 
	 * -A num <br>
	 * Sets the size of the kernel cache. Should be a prime number. (default
	 * 250007, use 0 for full cache)
	 * <p>
	 *
	 * -L num <br>
	 ...
	 *
	 * -V num <br>
	 * Number of folds for cross-validation used to generate data for logistic
	 * models. (default -1, use training data)
	 *
	 * -W num <br>
	...
	public void setOptions(String[] options) throws Exception {
		...
		if (nString.length() != 0) {
			setFilterType(new SelectedTag(Integer.parseInt(nString),
					TAGS_FILTER));
		} else {
		...
		if ((m_useRBF) && (m_featureSpaceNormalization)) {
			throw new Exception(
					"RBF machine doesn't require feature-space normalization.");
		}
		if ((m_exponent == 1.0) && (m_featureSpaceNormalization)) {
			throw new Exception(
					"Can't use feature-space normalization with linear machine.");
		}
		...
		if ((m_exponent == 1.0) && (m_lowerOrder)) {
			throw new Exception(
					"Can't use lower-order terms with linear machine.");
		}
	...
	public String[] getOptions() {
		...
		options[current++] = "-A";
		options[current++] = "" + m_cacheSize;
		options[current++] = "-L";
		options[current++] = "" + m_tol;
		options[current++] = "-P";
		options[current++] = "" + m_eps;
		options[current++] = "-N";
	...
	/**
	 * Set the value of exponent. If linear kernel is used, rescaling and
	 * lower-order terms are turned off.
	 *
	 * @param v
	...
	/**
	 * Set whether lower-order terms are to be used. Defaults to false if a
	 * linear machine is built.
	 * 
	 * @param v
	...
	public void mygenstat(Instances m_tinst) {
		...
		try {
			for (int m_k = 0; m_k < m_tinst.numInstances(); m_k++) {
				...
				String m_classname = mc_inst.stringValue(mc_inst
						.classAttribute());
	...
	public static void main(String[] argv) {
		...
		try {
			...
			if (m_trainFileName.length() != 0) {
				...
				m_trainReader = new BufferedReader(new FileReader(
						m_trainFileName));
			...
			if (m_testFileName.length() != 0) {
				...
				m_testReader = new BufferedReader(
						new FileReader(m_testFileName));
