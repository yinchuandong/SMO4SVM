...
public class Smo {
		...
		if ((r1 < -tolerance && alph1 < C) || (r1 > tolerance && alph1 > 0)) {
			{
				...
				for (i2 = (-1), tmax = 0, k = 0; k < end_support_i; k++)
					if (object2float(alph.elementAt(k)) > 0
							&& object2float(alph.elementAt(k)) < C) {
						float E2 = 0, temp = 0;
			...
			float rands = 0;
			{
				...
				for (rands = (float) Math.random(), k0 = (int) (rands * end_support_i), k = k0; k < end_support_i
						+ k0; k++) {

					i2 = k % end_support_i;

					if (object2float(alph.elementAt(i2)) > 0
							&& object2float(alph.elementAt(i2)) < C) {
						if (takeStep(i1, i2) == 1) {
	...
	int takeStep(int i1, int i2) {
...

		{
			...
			if (a1 > 0 && a1 < C)
				bnew = b + E1 + y1 * (a1 - alph1) * k11 + y2 * (a2 - alph2)
						* k12;
			else {
				if (a2 > 0 && a2 < C)
					...
					b1 = b + E1 + y1 * (a1 - alph1) * k11 + y2 * (a2 - alph2)
							* k12;
					b2 = b + E2 + y1 * (a1 - alph1) * k12 + y2 * (a2 - alph2)
							* k22;
					bnew = (b1 + b2) / 2;
				}
			}
		...
		if (is_linear_kernel) {
			...
			if (is_sparse_data && is_binary) {
				...
				num1 = ((sparse_binary_vector) sparse_binary_points
						.elementAt(i1)).id.size();

				for (p1 = 0; p1 < num1; p1++) {
					int temp0 = object2int(((sparse_binary_vector) sparse_binary_points
							.elementAt(i1)).id.elementAt(p1));
					float temp = object2float(w.elementAt(temp0));
					w.set(temp0, new Float(temp + t1));
				}
				num2 = ((sparse_binary_vector) sparse_binary_points
						.elementAt(i2)).id.size();

				for (p2 = 0; p2 < num2; p2++) {
					int temp0 = object2int(((sparse_binary_vector) sparse_binary_points
							.elementAt(i2)).id.elementAt(p2));
					float temp = object2float(w.elementAt(temp0));
				...
				for (p1 = 0; p1 < num1; p1++) {
					int temp1 = object2int(((sparse_vector) sparse_points
							.elementAt(i1)).id.elementAt(p1));
					float temp = object2float(w.elementAt(temp1));
					float temp2 = object2float(((sparse_vector) sparse_points
							.elementAt(i1)).val.elementAt(p1));
				...
				for (int i = 0; i < d; i++) {
					float temp = dense_points[i1][i] * t1 + dense_points[i2][i]
							* t2;
					;
		...
		// 更新误差
		{
			...
			for (int i = 0; i < end_support_i; i++)
				if (0 < object2float(alph.elementAt(i))
						&& object2float(alph.elementAt(i)) < C) {
					float tmp = object2float(error_cache.elementAt(i));
					tmp += t1 * kernel_func(i1, i, kernel_flag) + t2
							* kernel_func(i2, i, kernel_flag) - delta_b;
	...
	float learned_func_linear_sparse_binary(int k) {
		float s = 0;
		int temp = 0;
		for (int i = 0; i < ((sparse_binary_vector) sparse_binary_points
				.elementAt(k)).id.size(); i++)

		{
			temp = object2int(((sparse_binary_vector) sparse_binary_points
					.elementAt(i)).id.elementAt(i));
	...
	float learned_func_linear_sparse_nonbinary(int k) {
		...
		for (int i = 0; i < ((sparse_vector) sparse_points.elementAt(k)).id
				.size(); i++) {

			int j = object2int(((sparse_vector) sparse_points.elementAt(k)).id
					.elementAt(i));

			float v = object2float(((sparse_vector) sparse_points.elementAt(k)).val
					.elementAt(i));
	...
	float learned_func_nonlinear(int k) {
		...
		for (int i = 0; i < end_support_i; i++)
			if (object2float(alph.elementAt(i)) > 0) {
				s += object2float(alph.elementAt(i))
						* object2int(target.elementAt(i))
						* kernel_func(i, k, kernel_flag);
			}
		s -= b;
		return s;
	}
	...
	float dot_product_sparse_binary(int i1, int i2) {
		int p1 = 0, p2 = 0, dot = 0;
		int num1 = ((sparse_binary_vector) sparse_binary_points.elementAt(i1)).id
				.size();
		int num2 = ((sparse_binary_vector) sparse_binary_points.elementAt(i2)).id
				.size();

		while (p1 < num1 && p2 < num2) {
			int a1 = object2int(((sparse_binary_vector) sparse_binary_points
					.elementAt(i1)).id.elementAt(p1));
			int a2 = object2int(((sparse_binary_vector) sparse_binary_points
					.elementAt(i2)).id.elementAt(p2));
			if (a1 == a2) {
				dot++;
				p1++;
	...
	float dot_product_sparse_nonbinary(int i1, int i2) {
		...
		while (p1 < num1 && p2 < num2) {
			int a1 = object2int(((sparse_vector) sparse_points.elementAt(i1)).id
					.elementAt(p1));
			int a2 = object2int(((sparse_vector) sparse_points.elementAt(i2)).id
					.elementAt(p2));
			if (a1 == a2) {
				float val1 = object2float(((sparse_vector) sparse_points
						.elementAt(i1)).val.elementAt(p1));
				float val2 = object2float(((sparse_vector) sparse_points
						.elementAt(i2)).val.elementAt(p2));
		...
		try {
			for (n_lines = 0; (s = is.readLine()) != null; n_lines++) {
				...
				if (is_sparse_data && is_binary) {
					...
					for (int i = 0; i < n; i++) {
						if (object2float(v.elementAt(i)) < 1
								|| object2float(v.elementAt(i)) > d) {

							int line2 = n_lines + 1;
							System.out.println("error: line " + line2
									+ ": attribute index "
									+ (int) object2float(v.elementAt(i))
									+ " out of range.\n");
							System.exit(1);
						}
						x.id.add(new Integer(
								(int) object2float(v.elementAt(i)) - 1));
					}
					...
					if (this.is_libsvm_file) {
						for (int i = 0; i < n; i += 2) {
							if (object2float(v.elementAt(i)) < 1
									|| object2float(v.elementAt(i)) > d) {
								int line3 = n_lines + 1;
								System.out.println("data file error: line "
										+ line3 + ": attribute index "
										+ (int) object2float(v.elementAt(i))
										+ " out of range.\n");
								System.exit(1);
							}
							int id = (int) object2float(v.elementAt(i)) - 1;
							float value = (float) object2float(v
									.elementAt(i + 1));
					...
					if (v.size() != d) {
						...
						int line4 = n_lines + 1;
						System.out.println("Data file error: line " + line4
								+ " has " + v.size()
								+ " attributes; should be d=" + d);
						System.exit(1);
					}
					for (int i = 0; i < d; i++) {
			...
			for (int i = 0; i < end_support_i; i++)
				if (object2float(alph.elementAt(i)) > 0) {
					if (is_sparse_data && is_binary) {
						...
						os.print(" ");
						for (int j = 0; j < ((sparse_binary_vector) sparse_binary_points
								.elementAt(i)).id.size(); j++)

						{
							os.print(object2int(((sparse_binary_vector) sparse_binary_points
									.elementAt(i)).id.elementAt(j)) + 1);
							os.print(" ");
						}
					} else if (is_sparse_data && !is_binary) {
						os.print(object2int(target.elementAt(i)));
						os.print(" ");
						for (int j = 0; j < ((sparse_vector) sparse_points
								.elementAt(i)).id.size(); j++)

						{
							int id = object2int(((sparse_vector) sparse_points
									.elementAt(i)).id.elementAt(j)) + 1;
							float value = object2float(((sparse_vector) sparse_points
									.elementAt(i)).val.elementAt(j));
							os.print(id + " " + value + " ");
	...
	int read_svm(DataInputStream is) {
		...
		try {
			...
			is_sparse_data = Boolean.valueOf(is.readLine().toString())
					.booleanValue();
			is_binary = Boolean.valueOf(is.readLine().toString())
					.booleanValue();
			is_linear_kernel = Boolean.valueOf(is.readLine().toString())
					.booleanValue();
		...
		if (is_linear_kernel) {
			...
			for (int i = 0; i < d; i++) {
				try {
					float weight = Float.valueOf(is.readLine().toString())
							.floatValue();
			...
			try {
				two_sigma_squared = Float.valueOf(is.readLine().toString())
						.floatValue();
				int n_support_vectors = 0;
				n_support_vectors = Integer.valueOf(is.readLine().toString())
						.intValue();

				resize(alph, n_support_vectors, 2);

				for (int i = 0; i < n_support_vectors; i++) {
					float value = Float.valueOf(is.readLine().toString())
							.floatValue();
	...
	float error_rate() {
		...
		for (int i = first_test_i; i < N; i++) {
			if ((learned_func(i, learned_func_flag) > 0) != (object2int(target
					.elementAt(i)) > 0))
				n_error++;
			n_total++;
		}
...

			{
				...
				while ((ch = go.getopt()) != go.optEOF)
					...
					case 'c':
						...
						break;
					case 't':
						my.tolerance = go.processArg(go.optArgGet(),
								my.tolerance);
						break;
					case 'e':
						...
						break;
					case 'p':
						my.two_sigma_squared = go.processArg(go.optArgGet(),
								my.two_sigma_squared);
						break;
					case 'f':
...

			{
				...
				if (my.is_test_only) {
					...
					try {
						...
						my.end_support_i = my.first_test_i = n = my
								.read_svm(svm_file);
						// my.N += n;
					} catch (Exception e) {
						e.printStackTrace();
				...
				if (!my.is_test_only)
					if (my.N > 0) {
						...
						if (my.is_sparse_data && my.is_binary)
							my.reserveSparseBinary(my.sparse_binary_points,
									my.N);
			...
			if (!my.is_linear_kernel) {
				...
				for (int i = 0; i < my.N; i++)
					for (int j = 0; j < my.N; j++) {
						if (i != j)
							my.precomputed_dot_product[i][j] = my
									.dot_product_func(i, j, my.dot_product_flag);
						else {
							float temp = my.dot_product_func(i, i,
									my.dot_product_flag);
							my.precomputed_self_dot_product.set(i, new Float(
									temp));
			...
			if (!my.is_test_only) {
				...
				while (numChanged > 0 || examineAll > 0) {
					...
					if (examineAll > 0) {
						...
						for (int k = 0; k < my.N; k++) {
							if (my.object2float(my.alph.elementAt(k)) != 0
									&& my.object2float(my.alph.elementAt(k)) != my.C)
								numChanged += my.examineExample(k);
						}
					}
					if (examineAll == 1)
						...
						examineAll = 1;
					{
						...
						System.out.println("non_bound= " + non_bound_support
...

				{
					if (!my.is_test_only && svm_file_name != null) {
						try {
							PrintStream svm_file = new PrintStream(
									new FileOutputStream(svm_file_name));
			...
			newTime = System.currentTimeMillis();
			{
				try {
					...
					for (int i = my.first_test_i; i < my.N; i++) {
						svm_file.println(my.learned_func(i,
								my.learned_func_flag));
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
